...
from md.合并下载数据.下载全市场的实时数据 import get_full_tick
...
def getDayDataWithTimeout(stock_code = "600519.SH",  start_date = '20241101', end_date = "",is_download = 0, dividend_type = 'front'):
    """
    带超时和重试机制的getDayData包装函数
    
    接口与返回值与getDayData相同，负责调用getDayData函数。
    监测2秒超时，超时后重试3次，还不成功报错数据获取失败。
    
    Args:
        stock_code: 股票代码，如 "600519.SH"
        start_date: 开始日期，格式为 'YYYYMMDD'
        end_date: 结束日期，格式为 'YYYYMMDD'
        is_download: 是否下载数据，0为否，1为是
        dividend_type: 复权类型，'none'为不复权，'front'为前复权，'back'为后复权
        
    Returns:
        DataFrame: 股票K线数据，包含date列等
        
    Raises:
        TimeoutError: 3次重试后仍然超时，抛出"数据获取失败"错误
    """
    max_retries = 3
    timeout_seconds = 2
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # 检查是否是超时异常
            is_timeout = (isinstance(e, FutureTimeoutError) or 
                         'timeout' in error_type.lower() or 
                         'Timeout' in str(type(e)))
            
            if attempt < max_retries - 1:
                print(f"getDayData调用超时（第{attempt + 1}次尝试，耗时{elapsed:.1f}秒），1秒后重试...")
                time.sleep(1)
                continue
            else:
                print(f"✗ {stock_code} 数据获取失败（重试{max_retries}次后放弃）")
                if is_timeout:
                    raise TimeoutError(f"数据获取失败")
                else:
