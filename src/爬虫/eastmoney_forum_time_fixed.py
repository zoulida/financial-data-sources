...
class EastmoneyForumTimeCrawler:
    def __init__(self):
        ...
        for page in range(1, max_pages + 1):
            ...
            try:
                ...
                for i, link in enumerate(post_links):
                    try:
                        post_data = self.extract_post_data_final(link, page_text, page, i)
                        if post_data:
    ...
    def extract_post_data_final(self, link_element, page_text, page, index):
        """最终版本: 精确提取帖子数据"""
    ...
    def extract_counts_precise(self, context, title):
        ...
        for num in numbers:
            try:
                num_int = int(num)
                # 过滤掉明显不是阅读数/评论数的数字
                if 1 <= num_int <= 999999 and num_int != 601669:  # 排除股票代码
    ...
    def save_to_csv(self, posts, filename=None):
        """保存数据到CSV文件"""
        if not filename:
            filename = f"eastmoney_forum_posts_time_aligned_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def main():
    ...
    print(f"开始爬取 {stock_code} 的东方财富论坛数据（时间对齐版本）...")
