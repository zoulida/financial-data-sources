...
# -*- coding: utf-8 -*-
"""
中证500指数论坛爬虫 - 爬取zssh000300并筛选2024年1月1日后的数据
"""
...
class ZSSH000300Crawler:
    ...
    def crawl_forum_posts(self, stock_code="zssh000300", max_pages=10, start_date="2024-01-01"):
        """爬取论坛帖子数据并筛选指定日期后的数据"""
        all_posts = []
            ...
            try:
                ...
                for i, link in enumerate(post_links):
                    try:
                        ...
                        if post_data:
                            # 检查是否在指定日期之后
                            if self.is_after_start_date(post_data['post_time'], start_date):
    ...
    def is_after_start_date(self, post_time, start_date):
        """检查帖子时间是否在指定日期之后"""
        if not post_time:
            return False
        ...
        try:
            ...
            # 需要检查多个可能的年份：2024, 2025, 2026
            possible_years = [2024, 2025, 2026]
            
            for year in possible_years:
                # 创建帖子日期对象
                post_date = datetime(year, int(month), int(day))
    ...
    def save_to_csv(self, posts, filename=None):
        """保存数据到CSV文件"""
        if not filename:
            filename = f"zssh000300_forum_posts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def main():
    ...
    print(f"开始爬取 {stock_code} 的东方财富论坛数据（从 {start_date} 开始）...")
    
    crawler = ZSSH000300Crawler()
    
    # 爬取数据
    posts = crawler.crawl_forum_posts(stock_code=stock_code, max_pages=8, start_date=start_date)
