...
"""
新纳入股票表现分析 - 简化版
处理XtQuant API限制，使用Wind API作为备选方案
"""
...
class SimplifiedStockAnalyzer:
    ...
    def fetch_market_data_xtquant(self, stock_list, start_date, end_date):
        """
        使用XtQuant获取行情数据
        """
        print(f"正在使用XtQuant获取 {len(stock_list)} 只股票的行情数据")
        ...
        try:
            ...
            print(f"XtQuant获取数据失败: {e}")
            return None
    
    def fetch_market_data_wind(self, stock_list, start_date, end_date):
        """
        使用Wind API获取行情数据
        """
        print(f"正在使用Wind API获取 {len(stock_list)} 只股票的行情数据")
        
        try:
            # 使用Wind API获取数据
            data = w.wsd(stock_list, "close,volume", start_date, end_date, "")
            
            if data.ErrorCode != 0:
                print(f"Wind API获取数据失败，错误代码: {data.ErrorCode}")
                return None
            ...
            for i, stock in enumerate(stock_list):
                result[stock] = {
                    'close': data.Data[0][i] if len(data.Data) > 0 else [],
                    'volume': data.Data[1][i] if len(data.Data) > 1 else []
                }
            
            print("Wind API数据获取成功")
            return result
            
        except Exception as e:
            print(f"Wind API获取数据失败: {e}")
            return None
    
    def fetch_market_data(self, stock_list, start_date, end_date):
        """
        获取行情数据（优先使用XtQuant，失败则使用Wind）
        """
        if USE_XTQUANT:
            data = self.fetch_market_data_xtquant(stock_list, start_date, end_date)
            if data is not None:
                return data
        
        if USE_WIND:
            data = self.fetch_market_data_wind(stock_list, start_date, end_date)
            if data is not None:
                return data
        
        print("所有数据源都失败，无法获取行情数据")
        return None
    ...
    def calculate_performance_metrics(self, stock_code, market_data, trading_window, center_date):
        ...
        try:
            ...
            if USE_XTQUANT:
                ...
                # 设置日期索引（Wind API返回的是时间序列）
                if hasattr(market_data, 'Times'):
                    df_stock.index = pd.to_datetime(market_data.Times)
                else:
                    # 如果没有时间信息，使用交易日窗口
                    df_stock.index = pd.to_datetime(trading_window[:len(df_stock)])
...
def main():
    ...
    """
    print("=" * 60)
    print("中证新能源汽车产业指数新纳入股票表现分析 - 简化版")
    print("=" * 60)
    
    # 初始化分析器
    analyzer = SimplifiedStockAnalyzer('930997_changes_20240101_20251024.csv')
