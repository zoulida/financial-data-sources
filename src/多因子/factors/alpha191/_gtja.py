...
def IFELSE(condition: pd.DataFrame, A: Union[pd.DataFrame, Any], B: Union[pd.DataFrame, Any]) -> pd.DataFrame:
    if isinstance(A, pd.DataFrame):
        na_map = A.isna().astype(float).replace(1.0, np.nan)
        return A.where(condition, B) + na_map
    if isinstance(B, pd.DataFrame):
        na_map = B.isna().astype(float).replace(1.0, np.nan)
        return B.where(~condition, A) + na_map
    raise ValueError("A, B 必须有一个是 DataFrame")
...
def SMA(data: pd.DataFrame, n: int, m: int, ignore_nan: bool = True) -> pd.DataFrame:
    na_map = data.isna().astype(float).replace(1.0, np.nan)
    return data.ewm(alpha=m / n, ignore_na=ignore_nan).mean() + na_map


def _rolling_weighted(data: pd.DataFrame, weight: np.ndarray) -> pd.DataFrame:
    weight = weight / weight.sum()
    return data.rolling(len(weight)).apply(lambda values: float(np.dot(values, weight)), raw=True)


def WMA(data: pd.DataFrame, n: int) -> pd.DataFrame:
    return _rolling_weighted(data, np.arange(1, n + 1, dtype=float))


def DECAYLINEAR(data: pd.DataFrame, n: int) -> pd.DataFrame:
    return _rolling_weighted(data, np.arange(1, n + 1, dtype=float))


def DELTA(A: pd.DataFrame, n: int) -> pd.DataFrame:
    return A.diff(n)
...
def MEAN(price: pd.DataFrame, N: int) -> pd.DataFrame:
...
def COVARIANCE(A: pd.DataFrame, B: pd.DataFrame, n: int, sign: bool = False) -> pd.DataFrame:
    ...
    return np.sign(cov) if sign else cov


def MAX(A: pd.DataFrame, B: pd.DataFrame | float | int) -> pd.DataFrame:
    return np.maximum(A, B)


def MIN(A: pd.DataFrame, B: pd.DataFrame | float | int) -> pd.DataFrame:
    return np.minimum(A, B)


def HIGHDAY(A: pd.DataFrame, N: int, zero_diff: bool = False) -> pd.DataFrame:
    def _distance(values: np.ndarray) -> float:
        return float(len(values) - 1 - np.argmax(values))
    return A.rolling(N).apply(_distance, raw=True) - int(zero_diff)


def LOWDAY(A: pd.DataFrame, N: int, zero_diff: bool = False) -> pd.DataFrame:
    def _distance(values: np.ndarray) -> float:
        return float(len(values) - 1 - np.argmin(values))
    return A.rolling(N).apply(_distance, raw=True) - int(zero_diff)


def TS_MAX(A: pd.DataFrame, N: int) -> pd.DataFrame:
    return A.rolling(N).max()


def TS_MIN(A: pd.DataFrame, N: int) -> pd.DataFrame:
    return A.rolling(N).min()


def TS_RANK(data: pd.DataFrame, N: int) -> pd.DataFrame:
    return data.rolling(N).rank(pct=True)


def STD(price: pd.DataFrame, N: int) -> pd.DataFrame:
...
def rolling_slope(values: np.ndarray, period: int) -> np.ndarray:
    ...
    for i in range(period - 1, len(values)):
        ...
        if np.isnan(y).any() or denom == 0:
            continue
        result[i] = float(np.dot(x, y - y.mean()) / denom)
    return result


def rolling_resi(values: np.ndarray, period: int) -> np.ndarray:
    result = np.full(len(values), np.nan, dtype=float)
    x = np.arange(period, dtype=float)
    for i in range(period - 1, len(values)):
        y = values[i - period + 1:i + 1].astype(float)
        ...
        result[i] = float(y[-1] - (slope * x[-1] + intercept))
    return result


def rolling_rsquare(values: np.ndarray, period: int) -> np.ndarray:
    result = np.full(len(values), np.nan, dtype=float)
    x = np.arange(period, dtype=float)
    for i in range(period - 1, len(values)):
        y = values[i - period + 1:i + 1].astype(float)
        ...
        result[i] = np.nan if ss_tot == 0 else 1 - ss_res / ss_tot
    return result


def expanding_slope(values: np.ndarray) -> np.ndarray:
    result = np.full(len(values), np.nan, dtype=float)
    for i in range(1, len(values)):
        y = values[:i + 1].astype(float)
        if np.isnan(y).any():
            continue
        result[i] = float(np.polyfit(np.arange(i + 1, dtype=float), y, 1)[0])
    return result


def expanding_rsquare(values: np.ndarray) -> np.ndarray:
    result = np.full(len(values), np.nan, dtype=float)
    for i in range(1, len(values)):
        y = values[:i + 1].astype(float)
        if np.isnan(y).any():
...
def expanding_resi(values: np.ndarray) -> np.ndarray:
    result = np.full(len(values), np.nan, dtype=float)
    for i in range(1, len(values)):
        y = values[:i + 1].astype(float)
        if np.isnan(y).any():
...
def _alpha191_143(*args: Any, **kwargs: Any) -> pd.DataFrame:
    raise NotImplementedError("Alpha191 #143 在参考实现中标记为 unfinished，尚无可靠真实公式实现")
...
def alpha191_005(data, roll_tscorr=5, roll_tsmax=3):
    ...
    , 3))
    """
    tv = data["volume"].rolling(roll_tscorr).rank(pct=True)
    th = data["high"].rolling(roll_tscorr).rank(pct=True)
...
def alpha191_030(data):
    ...
    TODO
    """
    raise NotImplementedError("该因子在参考实现中标记为 unfinished，尚无可靠真实公式实现")
