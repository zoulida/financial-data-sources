...
def compute_mwega_1779417203_002(open_df, high_df, low_df, close_df, volume_df) -> pd.DataFrame:
    def _b(x):
    ...
    event_doji_like = (body_abs <= 0.008).astype(float)
    result = ((event_big_up).where((event_big_up) <= (event_doji_like), (event_doji_like)))
