...
def compute_mwega_1779417203_003(open_df, high_df, low_df, close_df, volume_df) -> pd.DataFrame:
    def _b(x):
    ...
    event_higher_low = (low_df >= low_df.shift(1).rolling(5, min_periods=3).min()).astype(float)
    result = (1.0 - _any(event_higher_low, 10))
