...
def compute_mwega_1779417203_005(open_df, high_df, low_df, close_df, volume_df) -> pd.DataFrame:
    def _b(x):
    ...
    event_big_bear_candle = ((ret1 <= -0.035) & (body_abs >= 0.025) & (close_df < open_df)).astype(float)
    event_big_up = (ret1 >= 0.045).astype(float)
    event_consecutive_down = ((ret1 < 0).rolling(3, min_periods=3).sum() >= 3).astype(float)
    event_near_limit_up = (ret1 >= 0.075).astype(float)
    result = ((event_big_bear_candle).where((event_big_bear_candle) <= (_then(_then(event_near_limit_up, (1.0 - _any(event_big_up, 3)), 3, 5), event_consecutive_down, 1, 10)), (_then(_then(event_near_limit_up, (1.0 - _any(event_big_up, 3)), 3, 5), event_consecutive_down, 1, 10))))
