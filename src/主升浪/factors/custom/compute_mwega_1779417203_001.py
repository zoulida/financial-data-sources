...
def compute_mwega_1779417203_001(open_df, high_df, low_df, close_df, volume_df) -> pd.DataFrame:
    def _b(x):
    ...
    event_break_ma20_down = (close_df < ma20 * 0.98).astype(float)
    result = ((event_big_up).where((event_big_up) <= (_then(event_break_ma20_down, (1.0 - _b(event_big_bull_candle)), 3, 3)), (_then(event_break_ma20_down, (1.0 - _b(event_big_bull_candle)), 3, 3))))
