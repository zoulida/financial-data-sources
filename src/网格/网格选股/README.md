...
步骤
取过去 N 日日内高价 H、低价 L，做成 2×N 的“区间带”。
当前价格 P 落在区间带的百分比 rank = (P−L_min)/(H_max−L_min)。
计算“极端分位+时间罚分”
Score = |rank − 0.5| × (1 − t/T_max)
t = 当前价格在此分位附近已停留的交易日数
T_max = 历史上价格曾在此分位停留的最大天数（滚动更新）
把 Score 从大到小排序，Score 越大→越可能很快回归，可直接当作“预期平仓天数”的代理；网格宽度按 Score 动态缩紧/放宽。
优点
完全非参，不估 β；
直观：rank 图一眼能看出“现在贵还是便宜”；
回测极快，1 分钟跑完全市场。

基于OLS离散回归计算均值回归速度：（已经弃用）
