...
class GridStrategy(CtaTemplate):
        ...
        if has_position_at_or_below:
            ...
            for i in range(self.spec.min_level_index, level_index + 1):
                ...
                if pos.qty > 0 and pos.avg_cost > 0:  # 确保有成本记录
                    ...
                    # 检查是否已经在目标卖出层级挂了卖单
                    if not self._has_pending(target_level, "SELL"):
                        sell_qty = pos.qty
                        sell_price = target_sell_price
                        
                        self.write_log(f"有仓位立即挂卖单: 买入层级{i} | 买入成本{pos.avg_cost:.6f} | 目标卖出价{sell_price:.6f} | 目标层级{target_level}")
                        self._place_order_real(target_level, "SELL", sell_qty, sell_price)
                        self._mark_pending(target_level, "SELL", sell_qty, sell_price)
                    else:
    ...
    def _print_real_order_status(self) -> None:
        """打印当前真实订单状态（包括券商订单ID）"""
        try:
            ...
            if real_orders:
                ...
                self.write_log("=== 真实订单状态结束 ===")
            else:
                self.write_log("=== 当前无真实订单（本地挂单状态可能未同步）===")
                
        except Exception as e:
            self.write_log(f"打印真实订单状态失败: {e}")
    ...
    def _has_real_buy_order_at_price(self, level_index: int) -> bool:
        """检查券商是否已经有指定层级的买单"""
        try:
            if not hasattr(self, 'manager') or not self.manager or not self.manager.trader:
                return False
            ...
            for order in orders_list:
                ...
                # 价格匹配（允许小数点后6位精度误差）
                is_price_match = abs(order_price - target_price) < 0.000001
                
                if is_match_stock and is_buy_order and is_price_match:
                    return True
            
            return False
            
        except Exception as e:
            self.write_log(f"检查券商买单状态失败: {e}")
            return False
    ...
    def _sync_order_status(self) -> None:
        ...
        try:
            # 1) 通过策略管理器查询券商真实订单状态
            has_manager = hasattr(self, 'manager') and self.manager
            has_trader_attr = has_manager and hasattr(self.manager, 'trader')
            has_trader = has_trader_attr and self.manager.trader
            
            #self.write_log(f"[DEBUG] manager检查: has_manager={has_manager}, has_trader_attr={has_trader_attr}, has_trader={has_trader}")
            
            if has_trader:
                # 调用base_trader_zld的get_unfilled_orders()获取未成交订单
                try:
                    orders_list = self.manager.trader.get_unfilled_orders()
                    # self.write_log(f"[DEBUG] 查询到未成交订单数量: {len(orders_list)}")
                    
                    if orders_list:
                        # 调试：打印所有订单的股票代码
                        stock_codes = [order.get('stock_code', '') for order in orders_list]
                        # self.write_log(f"[DEBUG] 未成交订单股票代码: {stock_codes}")
                        # self.write_log(f"[DEBUG] 当前策略股票代码: {self.manager.stock_code}")
                        ...
                        for order in orders_list:
                            ...
                            # 调试：打印每个订单的详细信息
                            status_desc = self._get_order_status_desc(order_status)
                            # self.write_log(f"[DEBUG] 订单详情: ID={order_id}, 代码={stock_code}, 状态={status_desc}, 数量={order_volume}, 成交={traded_volume}")
                            ...
                            # 支持多种匹配方式
                            is_match = (
                                stock_code == current_stock_clean or  # 513030 vs 513030
                                stock_code == current_stock or      # 513030 vs 513030.SH
                                stock_code + '.SH' == current_stock or  # 513030.SH vs 513030
                                stock_code + '.SZ' == current_stock     # 513030.SZ vs 513030
                            )
                            
                            if is_match:
                                # self.write_log(f"[DEBUG] 股票代码匹配成功: {stock_code} <-> {current_stock}")
                                remaining_volume = order_volume - traded_volume
                                
                                # self.write_log(f"[DEBUG] 未成交订单: 剩余数量={remaining_volume}")
                                ...
                                if remaining_volume > 0:
                                    ...
                                    if order_type == 23:
                                        ...
                                        # 如果order_type不可用，尝试从订单备注获取方向信息
                                        order_remark = order.get('order_remark', '')
                                        if 'BUY' in order_remark or 'buy' in order_remark:
                                            side = "BUY"
                                        elif 'SELL' in order_remark or 'sell' in order_remark:
                                            side = "SELL"
                                        else:
                                            side = "UNKNOWN"  # 无法确定方向
                                    ...
                                    # self.write_log(f"[DEBUG] 找到未成交订单: {order_id} {side} 价格{order_price} 数量{remaining_volume}")
                                else:
                                    # self.write_log(f"[DEBUG] 订单无剩余数量: {order_id}")
                                    # 空else块，不需要任何操作
                                    pass
                            else:
                                # self.write_log(f"[DEBUG] 股票代码不匹配: {stock_code} <-> {current_stock}")
                                pass  # 空else块
                        
                        # self.write_log(f"[DEBUG] 最终未成交订单数量: {len(real_orders)}")
                        ...
                        # 没有真实订单，清理所有本地挂单
                        # self.write_log("[DEBUG] 券商无挂单，清理所有本地挂单状态")
                        self._clear_all_pending_orders()
                        
                except Exception as e:
                    # self.write_log(f"[DEBUG] 查询券商订单失败: {e}")
                    import traceback
                    traceback.print_exc()
                    # 查询失败时，使用降级方案
                    self._cleanup_old_pending_orders(max_age_minutes=5)
            else:
                # 没有trader实例，使用降级方案
                # self.write_log("[DEBUG] 没有trader实例，使用降级方案")
                self._cleanup_old_pending_orders(max_age_minutes=5)
            
        except Exception as e:
            # self.write_log(f"[DEBUG] 同步订单状态失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _sync_with_real_orders(self, real_orders: dict) -> None:
        ...
        try:
            ...
            for (level_idx, side), details in self._pending_details.items():
                if (level_idx, side) in self._pending_orders:
                    ...
                    if local_order_id is None:
                        ...
                        if matched_order_id:
                            # 找到匹配的真实订单，更新订单ID
                            details['order_id'] = matched_order_id
                            self.write_log(f"补充本地订单ID: {side} 层级{level_idx} 价格{local_price} -> {matched_order_id}")
                        else:
                            # 没有找到匹配的真实订单，清理本地记录
                            local_to_remove.append((level_idx, side, "无对应真实订单"))
                    elif local_order_id not in real_orders:
                        # 本地订单ID在真实订单中不存在，清理本地记录
            ...
            for order_id, order_info in real_orders.items():
                ...
                price = round(order_info['price'], 6)  # 保留6位小数避免精度问题
                qty = order_info['qty']
                
                key = (price, side)
                if key not in price_side_orders:
                    price_side_orders[key] = {'total_qty': 0, 'orders': []}
                price_side_orders[key]['total_qty'] += qty
                price_side_orders[key]['orders'].append((order_id, qty))
            
            # 检查本地挂单是否已被真实订单覆盖
            for (level_idx, local_side), details in list(self._pending_details.items()):
                if (level_idx, local_side) not in self._pending_orders:
                    continue
                    
                local_price = round(details.get('price', 0), 6)
                local_qty = details.get('qty', 0)
                key = (local_price, local_side)
                
                if key in price_side_orders:
                    # 找到对应价格和方向的真实订单
                    real_total_qty = price_side_orders[key]['total_qty']
                    if real_total_qty >= local_qty:
                        # 真实订单数量足够覆盖本地挂单，更新订单ID
                        if details.get('order_id') is None:
                            # 取第一个订单ID作为代表
                            first_order_id = price_side_orders[key]['orders'][0][0]
                            details['order_id'] = first_order_id
                            self.write_log(f"更新本地订单ID: {local_side} 层级{level_idx} 价格{local_price} -> {first_order_id}")
                    
                    # 标记已处理的真实订单数量
                    processed_qty = min(local_qty, real_total_qty)
                    price_side_orders[key]['total_qty'] -= processed_qty
            
            # 报告未处理的真实订单，并添加到本地挂单状态
            for (price, side), info in price_side_orders.items():
                if info['total_qty'] > 0:
                    for order_id, qty in info['orders']:
                        if qty <= info['total_qty']:
                            self.write_log(f"发现未记录的真实订单: {order_id} {side} 价格{price} 数量{qty}")
                            
                            # 计算对应的网格层级
                            level_idx = int(round((price - self.spec.baseline) / self.spec.step))
                            ...
                            self.write_log(f"补充本地挂单状态: {side} 层级{level_idx} 价格{price} 订单{order_id}")
                            
                            info['total_qty'] -= qty
            
            # 3) 打印同步结果
            if local_to_remove:
                self.write_log(f"订单状态同步完成，清理了{len(local_to_remove)}个本地挂单")
            else:
                self.write_log("订单状态同步完成，本地状态与券商一致")
                
        except Exception as e:
            self.write_log(f"同步真实订单失败: {e}")
        ...
        try:
            ...
            try:
                ...
                for pos in positions:
                    ...
                    #self.write_log(f"[DEBUG] 检查仓位: 代码={stock_code}, 可用数量={pos.get('can_use_volume', 0)}, 总数量={pos.get('volume', 0)}")
                    if stock_code == self.manager.stock_code or stock_code == self.manager.stock_code.replace('.SH', '').replace('.SZ', ''):
                        # 使用正确的字段名：can_use_volume
                        broker_total_qty += pos.get('can_use_volume', 0)
