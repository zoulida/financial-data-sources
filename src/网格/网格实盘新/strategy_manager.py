...
if __package__ in {None, ""}:
    from src.网格.网格实盘新.grid_strategy import GridStrategy
    from src.网格.网格实盘新.order_sim import Trade
    from src.网格.网格实盘新.position_book import PositionBook
    from src.网格.网格实盘新.run_grid_512710 import _load_tick_raw_dataframe
...
class GridStrategyManager:
    ...
    def _init_qmt_trader(self) -> None:
        def on_filled(event: Dict[str, Any]):
            try:
                ...
                # 【修改】从meta中获取entry_id，传递给on_order_filled
                entry_id = meta.get("entry_id")
                
                # 调用策略的成交回调方法
                print(f"[DEBUG on_filled] 准备调用 on_order_filled: level={level_index}, side={side}, trade_id={trade_id_str}, entry_id={entry_id}")
                try:
                    self.strategy.on_order_filled(level_index, side, traded_price, traded_volume, trade_id_str, entry_id)
    ...
    def _place_real_order(self, level_index: int, side: str, qty: int, price: float, entry_id: str = None) -> None:
        """
        真实下单方法
        ...
        def _order_with_timeout():
            try:
                if side == "BUY":
                    # 【修改】买单备注使用entry_id，格式: BUY_entry_id
        ...
        try:
            # 兼容字符串/整数订单编号，映射两份键
            oid_str = str(oid)
            # 【修改】在_order_map中存储entry_id，用于成交回调时传递给on_order_filled
            self._order_map[oid_str] = {"level_index": level_index, "side": side, "qty": qty, "price": price, "entry_id": entry_id}
            try:
                oid_int = int(oid)
                self._order_map[oid_int] = {"level_index": level_index, "side": side, "qty": qty, "price": price, "entry_id": entry_id}
            except Exception:
                oid_int = None
            # 通知策略：挂单成功，带订单编号
            if self.strategy is not None:
