...
if __package__ in {None, ""}:
    from src.网格.网格实盘新.grid_engine import GridEngine, GridSpec
    from src.网格.网格实盘新.order_sim import Trade
    from src.网格.网格实盘新.position_book import PositionBook
    from src.网格.网格实盘新.reporter import Reporter
    from src.网格.网格实盘新.utils import within_trading_window
else:
...
class GridStrategy(CtaTemplate):
    ...
    def _handle_level_event(self, level_index: int, price: float, current_price: float) -> None:
        ...
        0.5、【新增】检查position文件中有entry_id但无buy_order_id的条目，查券商挂单补单或更新order_id
        ...
        # 【新增】0.5步骤：检查有entry_id但无buy_order_id的条目，补全券商订单号
        self._check_and_fill_missing_order_ids()
    ...
    def _place_order_real(self, level_index: int, side: str, qty: int, price: float) -> None:
        ...
        新逻辑：
        1. 下单前生成本地订单号(entry_id)
        2. 将entry_id写入position文件（buy_order_id暂时为空）
        3. 券商下单时备注写入本地订单号
        4. 下单成功后更新buy_order_id
        """
        if self._simulate_mode:
        ...
        if side == "BUY":
            ...
            # 先写入position文件，buy_order_id暂时为空
            entry = self.pos_book.add_buy(level_index, price, qty, buy_time, None, None)
            # 使用生成的entry_id覆盖自动生成的
            entry.entry_id = entry_id
            self.pos_book.save_to_csv()
            self.write_log(f"[预下单] 买单已写入position: entry_id={entry_id} | 层级={level_index} | 价格={price:.6f} | 数量={qty}")
        ...
        if self._order_placer is not None:
            # 【修改】传递entry_id给order_placer，用于券商备注
            self._order_placer(level_index, side, qty, price, entry_id)
    
    _entry_id_counter = 0  # 类级别自增计数器，防止同一微秒内重复
    
    def _generate_entry_id(self) -> str:
        """生成本地订单号（微秒时间戳 + 自增计数器，确保唯一）"""
        GridStrategy._entry_id_counter += 1
        ts = int(datetime.now().timestamp() * 1000000)  # 微秒级时间戳
        return f"{ts}_{GridStrategy._entry_id_counter}"
    ...
    def _check_and_fill_missing_order_ids(self) -> None:
        """
        检查position文件中有entry_id但无buy_order_id的条目
        通过券商备注(order_remark)匹配entry_id来补全buy_order_id
        
        逻辑：
        1. 遍历所有仓位记录，找出有entry_id但无buy_order_id的条目
        2. 查询券商未成交订单，通过order_remark中的entry_id匹配
        3. 匹配成功则更新buy_order_id
        4. 如果券商订单已成交（不在未成交列表中），查询当日成交记录匹配
        """
        try:
            # 获取所有缺少buy_order_id的仓位记录
            entries_missing_order_id = [
                e for e in self.pos_book.entries 
                if e.entry_id and not e.buy_order_id and e.sell_status != 'filled'
            ]
            
            if not entries_missing_order_id:
                return
            
            self.write_log(f"[订单补全] 发现{len(entries_missing_order_id)}条缺少buy_order_id的记录")
            
            # 检查是否有trader可用
            if not hasattr(self, 'manager') or not self.manager or not hasattr(self.manager, 'trader') or not self.manager.trader:
                self.write_log("[订单补全] 无法获取trader，跳过补全")
                return
            ...
            for order in orders_list:
                ...
                # order_remark格式应为: "BUY_entry_id" 或直接是entry_id
                if order_remark:
                    # 尝试提取entry_id
                    if order_remark.startswith('BUY_'):
                        extracted_entry_id = order_remark[4:]  # 去掉"BUY_"前缀
                    else:
                        extracted_entry_id = order_remark
                    
                    entry_to_order[extracted_entry_id] = str(order_id)
            
            # 更新缺少buy_order_id的记录
            updated_count = 0
            for entry in entries_missing_order_id:
                if entry.entry_id in entry_to_order:
                    entry.buy_order_id = entry_to_order[entry.entry_id]
                    updated_count += 1
                    self.write_log(f"[订单补全] 更新buy_order_id: entry_id={entry.entry_id} -> order_id={entry.buy_order_id}")
            
            if updated_count > 0:
                self.pos_book.save_to_csv()
                self.write_log(f"[订单补全] 已更新{updated_count}条记录的buy_order_id")
            
        except Exception as e:
            self.write_log(f"[订单补全] 检查失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def on_order_filled(self, level_index: int, side: str, fill_price: float, qty: int, trade_id: str = None, entry_id: str = None) -> None:
        """
        订单成交回调 - 新仓位逻辑
        
        买入成交时：
           - 【修改】仓位记录已在_place_order_real中预先添加，这里只更新buy_order_id和buy_trade_id
           - 通过entry_id找到对应的仓位记录并更新
        ...
        try:
            ...
            if side == "BUY":
                # 买入成交：【修改】仓位记录已预先添加，这里只更新buy_order_id和buy_trade_id
                ...
                entry = None
                if entry_id:
                    entry = self.pos_book.get_entry_by_id(entry_id)
                
                if entry:
                    # 更新已有仓位记录的buy_order_id和buy_trade_id
                    entry.buy_order_id = order_id_str
                    entry.buy_trade_id = trade_id_str
                    # 【新增】买单成交后，状态改为BuyFilled，表示可以挂卖单了
                    ...
                    # 如果找不到对应的仓位记录（兼容旧逻辑），则新建
                    now = datetime.now()
                    buy_time = now.strftime("%Y-%m-%d %H:%M:%S")
                    entry = self.pos_book.add_buy(level_index, fill_price, qty, buy_time, order_id_str, trade_id_str)
                    # 【新增】新建的仓位也设置为BuyFilled
                    entry.sell_status = 'BuyFilled'
                    self.write_log(f"买单成交(新建): 层级{level_index} | 价格{fill_price:.6f} | 数量{qty} | 委托{order_id} | 成交{trade_id} | 仓位ID:{entry.entry_id}")
    ...
    def _sync_positions_with_broker(self) -> None:
        ...
        try:
            ...
            # 【新增】记录被删除的仓位到日志文件
            removed_entries = [e for e in self.pos_book.entries if e.sell_status == 'filled']
            valid_entries = [e for e in self.pos_book.entries if e.sell_status != 'filled']
            removed_count = initial_count - len(valid_entries)
            
            if removed_count > 0:
                # 记录删除日志到文件
                self._log_removed_positions(removed_entries, "filled状态清理")
    ...
    def _check_positions_on_tick(self, current_price: float) -> None:
        ...
        """
        try:
            # 0. 同步买单状态：pending且有buy_order_id的仓位，查券商确认是否已成交
            self._sync_buy_order_status()
            ...
            # 3. 为未挂卖单的仓位挂卖单（BuyFilled + cancelled）
            self._place_sell_for_pending_positions()
            
        except Exception as e:
            self.write_log(f"tick仓位检查失败: {e}")
    
    def _sync_buy_order_status(self) -> None:
        """同步买单状态
        
        检查pending状态且有buy_order_id的仓位，查券商确认买单是否已成交:
        - 已成交(56) -> BuyFilled（后续由_place_sell_for_pending_positions挂卖单）
        - 已撤销(54) -> 删除该仓位记录
        - 已报(50)/部成(55) -> 保持pending
        """
        if not hasattr(self, 'manager') or not self.manager or not hasattr(self.manager, 'trader') or not self.manager.trader:
            return
        ...
        try:
            ...
            self.write_log(f"同步买单状态失败: {e}")
    
    def _sync_sell_order_status(self) -> None:
        """同步卖单状态
        
        根据券商订单状态更新本地仓位的sell_status:
        - 已成交(56) -> filled
        - 已撤销(54) -> cancelled  
        - 已报/其他 -> hanging (保持不变)
        """
        ...
        try:
            # 【修复】使用get_orders获取所有订单（包括已成交），而非get_unfilled_orders
            orders_list = self.manager.trader.get_orders()
            if not orders_list:
                orders_list = []
            
            # 获取所有已挂卖单的仓位（hanging状态）
            ...
            for entry in hanging_entries:
                ...
                for order in orders_list:
                    ...
                    if order_id == str(entry.sell_order_id):
                        order_found = True
                        # 订单状态码: 48=未报, 49=待报, 50=已报, 51=已报待撤, 52=部成待撤, 
                        # 53=部撤, 54=已撤, 55=部成, 56=已成, 57=废单
                        status = order.get('order_status', 0)
                        if status == 56:  # 已成交
                            self.pos_book.mark_sell_filled(entry.entry_id)
                            self.write_log(f"卖单已成交(同步): 仓位ID={entry.entry_id} | 卖单号={entry.sell_order_id}")
                        elif status == 54:  # 已撤销
                            entry.sell_status = 'cancelled'
                            self.write_log(f"卖单已撤销: 仓位ID={entry.entry_id} | 状态->cancelled")
                        elif status in (50, 55):  # 已报 或 部成
                            # 保持hanging状态
                            pass
                        break
                ...
                # 订单不在列表中，可能是跨日订单或异常
                if not order_found:
                    self.write_log(f"[警告] 卖单不在订单列表: 仓位ID={entry.entry_id} | 卖单号={entry.sell_order_id}")
    ...
    def _check_old_positions_and_sell(self) -> None:
        ...
        for entry in old_entries:
            if entry.sell_status in ('pending', 'BuyFilled'):
                # 非今日仓位还没有挂卖单，需要挂卖单
                # 【修改】pending是买单未成交，BuyFilled是买单已成交待挂卖单
                self.write_log(f"发现非今日仓位: 仓位ID={entry.entry_id} | 买入日期={entry.buy_date} | 状态={entry.sell_status}")
    
    def _place_sell_for_pending_positions(self) -> None:
        """为买单已成交的仓位挂卖单，检查券商可用仓位，优先挂买入价高的仓位
        
        处理以下状态的仓位:
        - BuyFilled: 买单已成交，等待挂卖单
        - cancelled: 卖单被撤销，需要重新挂卖单
        """
        # 获取需要挂卖单的仓位（BuyFilled + cancelled）
        buy_filled_entries = self.pos_book.get_buy_filled_entries()
        ...
        for entry in entries_to_sell:
            ...
            if order_id:
                # 【修复】无论部分还是全部挂单，都要更新卖单信息（状态改为hanging）
    ...
    def _place_sell_order_for_entry_partial(self, entry: 'PositionEntry', sell_price: float, sell_level: int, qty: int) -> Optional[str]:
        ...
        【修改】卖出时备注写入对应买单的entry_id，格式: SELL_entry_id
        """
        try:
            if self._simulate_mode:
                # 模拟模式：生成模拟订单号
                ...
                # 实盘模式：真实下单
                if hasattr(self, 'manager') and self.manager and hasattr(self.manager, 'trader') and self.manager.trader:
                    # 构建备注，包含对应买单的entry_id，格式: SELL_entry_id
                    order_remark = f"SELL_{entry.entry_id}" if entry.entry_id else ""
    ...
    def _log_removed_positions(self, entries: list, reason: str) -> None:
        ...
        try:
            ...
            self.write_log(f"[删除日志] 已记录{len(entries)}条被删除仓位到: {log_file}")
        except Exception as e:
